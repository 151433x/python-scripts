...
def placement_generator(df,brand,quarter,year):
    ...
    for i in df.iloc:
        ...
        if i[2]=='Visits' and i[11]=='Display': # we want to analyze store visits on targeting detail level so this allows us to only generate the placement names for display visits and not others
...
def final(name,quarter,year):
    """ 
        The function final takes in name(the brand or that you would like to search fore), quarter(the quarter of the data we are compiling) and year (the year of the data).
